...
const { getDatabase, ref, get, set, onValue, off } = window.firebaseDatabase ?? {};

let _db = null;
function getDB() {
  if (!_db) {
    const app = initializeApp(YOUR_FIREBASE_CONFIG, "chatroom");
    _db = getDatabase(app);
  }
  return _db;
}
...
async function sset(path, val) {
  try { await set(ref(getDB(), path), val); } catch {}
}

const loadMessages  = () => sget("messages").then(v => v ? Object.values(v).sort((a,b)=>a.ts-b.ts) : []);
const saveMessages  = async (msgs) => {
  const trimmed = msgs.slice(-MAX);
  const obj = {};
  trimmed.forEach(m => { obj[m.id.replace(/[.#$\[\]]/g,"-")] = m; });
  await sset("messages", obj);
};
const loadInvites   = () => sget("invites").then(v => v ?? {});
const loadMembers   = () => sget("members").then(v => v ?? {});
const loadPubKeys   = () => sget("pubkeys").then(v => v ?? {});
...
export default function App() {
  ...
  async function handleLogin() {
    ...
    await sset("pubkeys", pubKeys);

    // Mark invite used
    if (validatedCode && validatedCode !== "FOUNDER") {
      const invites = await loadInvites();
      if (invites[validatedCode]) { invites[validatedCode].used = true; invites[validatedCode].usedBy = sanitized; await sset("invites", invites); }
    }
    const invitedBy = null;
    existingMembers[sanitized] = { joinedAt: Date.now(), invitedBy };
    await sset("members", existingMembers);
